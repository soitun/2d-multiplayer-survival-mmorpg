...
pub fn check_monument_zone_placement(ctx: &ReducerContext, world_x: f32, world_y: f32) -> Result<(), String> {
    ...
    const CRASHED_RESEARCH_DRONE_RESTRICTION_RADIUS: f32 = 800.0; // Tundra monument (800px minimum)
    ...
    const WEATHER_STATION_RESTRICTION_RADIUS: f32 = 2000.0; // Four radars, central compound, 2000px exclusion
    ...
    // Check reed marsh zones (environmental monuments in rivers - tern hunting/reed collection)
    // Building restriction radius matches the marsh radius stored in database
    for marsh in ctx.db.reed_marsh().iter() {
        let dx = world_x - marsh.world_x;
        let dy = world_y - marsh.world_y;
        let distance_sq = dx * dx + dy * dy;
        let marsh_radius_sq = marsh.radius_px * marsh.radius_px;
...
pub fn place_foundation(
    ...
    // 5.1. Check if position is on asphalt (compounds) - cannot build on paved areas
    ...
    let foundation_min_y = world_y - foundation_half_size;
    let foundation_max_y = world_y + foundation_half_size;
    ...
    let chunk_size_px = crate::environment::CHUNK_SIZE_PX;
    let chunks_per_row = crate::environment::WORLD_WIDTH_CHUNKS;
    
    // Check if we're near chunk boundaries and need to check adjacent chunks
    let local_x = world_x % chunk_size_px;
    let local_y = world_y % chunk_size_px;
    ...
    // 10. Destroy any planted seeds and harvestable resources within the foundation bounds
    // Foundation is 96x96 pixels centered at (world_x, world_y)
    let foundation_half_size = FOUNDATION_TILE_SIZE_PX as f32 / 2.0;
    let found_min_x = world_x - foundation_half_size;
    let found_max_x = world_x + foundation_half_size;
    let found_min_y = world_y - foundation_half_size;
    let found_max_y = world_y + foundation_half_size;
    
    // Destroy planted seeds within foundation bounds
    let planted_seeds_to_destroy: Vec<u64> = ctx.db.planted_seed().iter()
        .filter(|seed| {
            seed.pos_x >= found_min_x && seed.pos_x <= found_max_x &&
            seed.pos_y >= found_min_y && seed.pos_y <= found_max_y
        })
        .map(|seed| seed.id)
        .collect();
    
    for seed_id in &planted_seeds_to_destroy {
        ctx.db.planted_seed().id().delete(*seed_id);
    }
    
    if !planted_seeds_to_destroy.is_empty() {
        log::info!(
            "[PlaceFoundation] Destroyed {} planted seeds at cell ({}, {})",
            planted_seeds_to_destroy.len(), cell_x, cell_y
        );
    }
    
    // Destroy harvestable resources within foundation bounds (only active ones, not respawning)
    let resources_to_destroy: Vec<u64> = ctx.db.harvestable_resource().iter()
        .filter(|resource| {
            // Only destroy active resources (respawn_at == UNIX_EPOCH means active)
            resource.respawn_at == spacetimedb::Timestamp::UNIX_EPOCH &&
            resource.pos_x >= found_min_x && resource.pos_x <= found_max_x &&
            resource.pos_y >= found_min_y && resource.pos_y <= found_max_y
        })
    ...
    if !resources_to_destroy.is_empty() {
        log::info!(
            "[PlaceFoundation] Destroyed {} harvestable resources at cell ({}, {})",
            resources_to_destroy.len(), cell_x, cell_y
        );
    }
    
    // 11. Emit foundation construction sound for all nearby players
